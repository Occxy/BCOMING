...
var valid_field = 
	['Project', 'Sampling_Date', 'Animal_ID', 'Genus_Species', 'Province', 'District', 'Site', 'Specific_location', 'Sex', 'Age-Status', 'Health_Status', 'Condition', 'FEV', 'OSV', 'OST', 'RSV', 'RST', 'DBS', 'URV', 'URT', 'WBV', 'BSN', 'BCN', 'FA_mm', 'Weigth_g', 'Photo', 'Recorder_type', 'File_name', 'Kiv', 'KiT', 'SpV', 'SpT', 'LiV', 'LiT', 'LuV', 'LuT', 'HeV', 'HeT', 'BrV', 'BrT', 'UrV', 'UrT', 'Other'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_RhinoKhov_ZooCov_Animal(line);
				};
			/*} else {
...
function fill_tab_datas_RhinoKhov_ZooCov_Animal(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_RhinoKhov_ZooCov_Animal(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_RhinoKhov_ZooCov_Animal(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_RhinoKhov_ZooCov_Animal(id, i);
		    			...
		    			search_N_identification_Recursif_RhinoKhov_ZooCov_Animal(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_RhinoKhov_ZooCov_Animal(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_RhinoKhov_ZooCov_Animal(/*localDB, */i-1);
		...
		synchronizeRhinoKhov_ZooCov_Animal();
		/*if (localStorage.getItem('web') !== 'oui') {
...
function put_with_id_RhinoKhov_ZooCov_Animal(id, i) {
...
function put_new_id_RhinoKhov_ZooCov_Animal(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeRhinoKhov_ZooCov_Animal() {
