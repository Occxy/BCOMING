var fields = 
    			...
    			['Code', 'Date', 'Poids_g', 'Poids_avant_sechage', 'Username'] 
    			 
    		


var fields_CSV_head =   'Code;Date;Poids_g;Poids_avant_sechage;Username;\r\n';
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DBinsula_he_samples = new PouchDB(remote_couchdb + 'bcoming' + nom_table + debug);
} else {
	var DBinsula_he_samples = new PouchDB('bcoming' + nom_table + debug);
};

var tab_insula_he_samples = new Array();
var tab = new Array();


DBinsula_he_samples.allDocs({  		
	include_docs: true,
	attachments: true
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				...
   				obj.Poids_avant_sechage = row.doc.Poids_avant_sechage
   				
   				obj.Username = row.doc.Username 

   				tab_insula_he_samples.push(obj);
	...
	DBinsula_he_samples.info().then((infos) => {
...
function addInsula_he_samplesRecord(row, selected) {
		...
		if (!selected) { 	
			...
			if (count == 0) {
				var blob = new Blob(['\ufeff' + CSV_data], {type: "text/csv;charset=ISO-8859-1"});
	       		saveAs(blob, "insula_he_samples" + clock.now + ".csv");
...
window.onload = function() {
	...
	$('#export_all_fields').click(function(){
		...
		var res = alasql("SELECT Code, Date, Poids_g, Poids_avant_sechage, Username FROM ? ORDER BY Date", [tab_insula_he_samples] );
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addInsula_he_samplesRecord(row, false);
			}
	...
	$('#export_selected_fields').click(function(){
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addInsula_he_samplesRecord(row, true);
			}
