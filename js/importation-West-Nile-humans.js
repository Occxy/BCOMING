...
var valid_field = 
	['Date_prelevement', 'Description', 'Organism', 'Age', 'Comunne', 'Colonne1', 'Lieu_prelevement', 'WNV_IgM_CNR', 'WNV_IgG_CNR', 'Autres_arboviroses'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_WewtNile_humans(line);
				};
			/*} else {
...
function fill_tab_datas_WewtNile_humans(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_WewtNile_humans(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_WewtNile_humans(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_WewtNile_humans(id, i);
		    			...
		    			search_N_identification_Recursif_WewtNile_humans(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_WewtNile_humans(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_WewtNile_humans(/*localDB, */i-1);
...
function put_with_id_WewtNile_humans(id, i) {
...
function put_new_id_WewtNile_humans(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeWewtNile_humans() {
