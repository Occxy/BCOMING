function chargement_insula_av_inventories_wp1() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('InsulaAVinventories_wp1TablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Scientific_Name');
				addValue('Common_Name');
				addValue('Count');
				addValue('Location_Code');
				addValue('Location_Site');
				addValue('Latitude');
				addValue('Longitude');
				addValue('Protocol');
				addValue('Duration_Min');
