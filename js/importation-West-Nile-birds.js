...
var valid_field = 
	['Date_prelevement', 'Description', 'Commune', 'Organism', 'Nom_demandeur', 'Statut_migratoire', 'Essai', 'N_de_res', 'Resultat', 
		'Confirmation', 'Resultat_conf', 'Lon', 'Lat'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_WewtNile_birds(line);
				};
			/*} else {
...
function fill_tab_datas_WewtNile_birds(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_WewtNile_birds(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_WewtNile_birds(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_WewtNile_birds(id, i);
		    			
		    			for(var rowCountContent = 0; rowCountContent < rowContent_length; rowCountContent++) {
							var rowContent = tab[i][rowCountContent]
							
							if (rowCountContent == 11 || rowCountContent == 12) {
							...
							if ((rowCountContent == 11 || rowCountContent == 12) && detectCoordinateFormat(rowContent) == 'DMS') {
								const convertedCoordinate = convertDMSStringToDecimal(rowContent);
					            if (convertedCoordinate !== null) {
					            	
					                rowContent = convertedCoordinate.toString();
					                
					                //tab[i][rowCountContent] = rowContent; // Mettre à jour le tableau principal
					            }
					        }
		    			...
		    			search_N_identification_Recursif_WewtNile_birds(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_WewtNile_birds(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_WewtNile_birds(/*localDB, */i-1);
...
function put_with_id_WewtNile_birds(id, i) {
...
function put_new_id_WewtNile_birds(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeWewtNile_birds() {
...
function detectCoordinateFormat(coordinateString) {
	let aUneApostrophe = coordinateString.includes("'");
	

	if (aUneApostrophe) {
		return 'DMS'; // Format degrés, minutes, secondes
    } else {
        return 'Unknown'; // Format inconnu
    }
...
function convertDMSStringToDecimal(dmsString) {
    ...
    return null; // Format inconnu ou incomplet
}
