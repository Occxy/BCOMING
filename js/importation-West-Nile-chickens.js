...
var valid_field = 
	['Date_prelevement', 'Description', 'Espece', 'Type_d_enquete', 'Nom_demandeur', 'Nom_d_eleveur', 'Commune', 'N_de_suivi', 'Essai',
		'N_de_res', 'Resultat', 'Confirmation', 'Resultat_conf', 'Lon', 'Lat'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_WestNile_horses(line);
				};
			/*} else {
...
function fill_tab_datas_WestNile_horses(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_WestNile_horses(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_WestNile_horses(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			/*if ((tab[i][0] == '') && (tab[i][1] == '') && (tab[i][5] == '') && (tab[i][8] == '') && (tab[i][9] == '')) {
				search_N_identification_Recursif_WestNile_horses(i-1);
				...
				put_new_id_WestNile_horses(new_doc/*, i*/);
				
				move();
				row++;
				
				search_N_identification_Recursif_WestNile_horses(/*localDB, */i-1);
			//}
		    		
		}   	
	} else {
		
		synchronizeWestNile_horses();
		
	};
};

function put_with_id_WestNile_horses(id, i) {
...
function put_new_id_WestNile_horses(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
	...
	DB.put(doc).then(function () {
	    ...
	    console.error('Erreur lors de l\'opération put :', err);
	    // Autres actions à entreprendre en cas d'erreur...
	  });
};

function synchronizeWestNile_horses() {
