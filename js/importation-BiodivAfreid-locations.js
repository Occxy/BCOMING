		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_BiodivAfreid_locations(line);
				};
			/*} else {
...
function fill_tab_datas_BiodivAfreid_locations(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_BiodivAfreid_locations(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_BiodivAfreid_locations(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			location_code = tab[i][0];
			
			DB.find({
		       	selector: {location_code: location_code}
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_BiodivAfreid_locations(id, i);
		    			...
		    			search_N_identification_Recursif_BiodivAfreid_locations(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_BiodivAfreid_locations(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_BiodivAfreid_locations(/*localDB, */i-1);
		...
		synchronizeBiodivAfreid_locations();
		/*if (localStorage.getItem('web') !== 'oui') {
...
function put_with_id_BiodivAfreid_locations(id, i) {
...
function put_new_id_BiodivAfreid_locations(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table +  debug);
	} else {
...
function synchronizeBiodivAfreid_locations() {
