...
var valid_field = 
	['ID_FD', 'ID_FS', 'Group', 'Species', 'Sex', 'Individuals', 'Used_Individuals', 'Date_Used_Individuals', 'RNA_and_cDNA_plates_ANSES', 
		'RNA_and_cDNA_well', 'RNA_and_cDNA_Id', 'Date', 'Year', 'Month', 'Day', 'Site', 'Location', 'Habitat', 
		'Colonne1', 'Observation', 'Notes', 'Lat', 'Lon', 'Project_Study', 'Trap_method', 'Biotope'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_WestNile_mosquitoes_field_samples(line);
				};
			/*} else {
...
function fill_tab_datas_WestNile_mosquitoes_field_samples(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_WestNile_mosquitoes_field_samples(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_WestNile_mosquitoes_field_samples(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_WestNile_mosquitoes_field_samples(id, i);
		    			
		    			
						
						//addEspeceInTableReferenceBatCapturees(i);
		    			
		    			search_N_identification_Recursif_WestNile_mosquitoes_field_samples(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
		    		...
		    		for(var rowCountContent = 0; rowCountContent < rowContent_length; rowCountContent++) {
						...
						if (((rowCountContent == 7) || (rowCountContent == 11)) && (tab[i][rowCountContent] != '') && (tab[i][rowCountContent] != null) && ((tab[i][rowCountContent]).length !==4) && ((tab[i][rowCountContent]).length !==6) ){
							// Chaîne représentant la date au format AAAAJJMM
							var date = tab[i][rowCountContent];

							// Extraction de l'année, du mois et du jour de la chaîne
							var annee = date.substring(0, 4);
							var mois = date.substring(4, 6);
							var jour = date.substring(6, 8);

							// Construction de la date dans le format JJ/MM/AAAA
							var rowContent = jour + '/' + mois + '/' + annee;
						} else {
							if ((tab[i][rowCountContent] == null) || ((tab[i][rowCountContent] == '') && ((rowCountContent == 0))) || (tab[i][rowCountContent] == '#REF!'))  {
								var rowContent = 'undefined';
							} else if (((tab[i][rowCountContent]).length ===6) && ((rowCountContent == 7) || (rowCountContent == 11))) {
								var date = tab[i][rowCountContent];
					...
					put_new_id_WestNile_mosquitoes_field_samples(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_WestNile_mosquitoes_field_samples(/*localDB, */i-1);
		...
		synchronizeWestNile_mosquitoes_field_samples();
		/*if (localStorage.getItem('web') !== 'oui') {
...
function put_with_id_WestNile_mosquitoes_field_samples(id, i) {
...
function put_new_id_WestNile_mosquitoes_field_samples(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeWestNile_mosquitoes_field_samples() {
