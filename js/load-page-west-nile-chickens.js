function chargement_west_nile_chickens() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('WestNileChickensTablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Description');
				addValue('Espece');
				addValue('Type_d_enquete');
				addValue('Nom_demandeur');
				addValue('Nom_d_eleveur');
				addValue('Commune');
				addValue('N_de_suivi');
				addValue('Essai');
				addValue('N_de_res');
