...
function fill_tab_datas_CamAcross(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_CamAcross(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_CamAcross(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_CamAcross(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_CamAcross(new_doc/*, i*/);
...
function put_with_id_CamAcross(id, i) {
...
function put_new_id_CamAcross(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
