var rhinokhov_zoocov_animal_table = localStorage.getItem('rhinokhov_zoocov_animal_table');
...
$(document).ready(function() {			
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + rhinokhov_zoocov_animal_table + debug);
	} else {
		var DB = new PouchDB(rhinokhov_zoocov_animal_table + debug);
	};	
  	DB.allDocs({
		include_docs: true,
		attachments: true
		...
		if (typeof(JSON.stringify(result)) != "undefined"){  
    		...
    		dataTablesData.rows.forEach(function(row){   
    			table.row.add(new RhinoKhov_ZooCov_animal(row.doc.Sampling_Date, row.doc.Animal_ID, row.doc.Genus_Species, row.doc.Province, row.doc.Site,  
...
function RhinoKhov_ZooCov_animal(Sampling_Date, Animal_ID, Genus_Species, Province, Site, Action) {
