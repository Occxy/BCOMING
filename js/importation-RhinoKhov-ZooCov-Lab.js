...
var valid_field = 
	['Project', 'No', 'Sampling_Date', 'Animal_ID', 'Sample_Type', 'Taxa', 'Genus_Species', 'Province', 'rtPCR_SARS-COV-2_E_gene', 'PCR_Quan_result', 'PCR_Watanabe_result'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_RhinoKhov_ZooCov_Lab(line);
				};
			/*} else {
...
function fill_tab_datas_RhinoKhov_ZooCov_Lab(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_RhinoKhov_ZooCov_Lab(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_RhinoKhov_ZooCov_Lab(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_RhinoKhov_ZooCov_Lab(id, i);
		    			...
		    			search_N_identification_Recursif_RhinoKhov_ZooCov_Lab(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_RhinoKhov_ZooCov_Lab(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_RhinoKhov_ZooCov_Lab(/*localDB, */i-1);
		...
		synchronizeRhinoKhov_ZooCov_Lab();
		/*if (localStorage.getItem('web') !== 'oui') {
...
function put_with_id_RhinoKhov_ZooCov_Lab(id, i) {
...
function put_new_id_RhinoKhov_ZooCov_Lab(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeRhinoKhov_ZooCov_Lab() {
