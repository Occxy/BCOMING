function chargement_west_nile_horses() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('WestNileHorsesTablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Confirmation');
				addValue('Resultat_conf');
				addValue('Seroconversion_prev');
				addValue('Changer_status');
				addValue('Essai_IgM');
				addValue('Resultat_IgM');
