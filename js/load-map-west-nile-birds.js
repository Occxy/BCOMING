...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_west_nile_birds' + debug);
} else {
	var DB = new PouchDB('bcoming_west_nile_birds' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
 			try {
 				var obj = new Object();
 				obj.Organism  = row.doc.Organism.trim() ;
	...
	resWest_nile_birds = alasql('SELECT Lon, Lat, Commune, Organism,  COUNT(*) AS _Count FROM ? GROUP BY  Lon, Lat, Commune, Organism ORDER BY Lon, Lat, Commune, Organism', [tab]);
	console.log(resWest_nile_birds);

	var groupedData = {};

	resWest_nile_birds.forEach(function(entry) {
	    var organism = entry.Organism;
	    var commune = entry.Commune;
	    var latitude = entry.Lat;
	    var longitude = entry.Lon;
	    var count = entry._Count;

	    var key = latitude + '_' + longitude + '_' + organism + '_' + commune;
	...
	for (var key in groupedData) {
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	/*var sites = resWest_nile_birds.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    	    // Utilisez les sites trouvés pour créer les bulles
	    	    sites.forEach(site => {
	    	        siteTitle = site.Site_organism + '/' + site.Lat + '/' + site.Long;
	    ...
	    contentString += '<p>Organism: ' + organism + ', Count: ' + groupedData[key] + '</p>';
	    contentString += '</div>';
