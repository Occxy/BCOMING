...
var valid_field = 
	...
	['Sample', 'Date', 'Observateurs', 'Site', 'Def_site', 'Biotope', 'Taxa', 'Famille', 'Genre', 'Species', 'Poids_g', 'Taille_queue_cm', 'Taille_corps_cm', 'Sexe', 'Code'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_MI_Samples(line);
				};
			/*} else {
...
function fill_tab_datas_MI_Samples(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_MI_Samples(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_MI_Samples(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_MI_Samples(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_MI_Samples(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_MI_Samples(/*localDB, */i-1);
...
function put_with_id_MI_Samples(id, i) {
...
function put_new_id_MI_Samples(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeMI_Samples() {
