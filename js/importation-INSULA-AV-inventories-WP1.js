...
var valid_field = 
	['Common_Name', 'Scientific_Name', 'Count', 'Location_Code', 'Location_Site', 'Latitude', 'Longitude', 'Date', 'Time', 'Protocol', 'Duration_Min'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_AV_inventories_WP1(line);
				};
			/*} else {
...
function fill_tab_datas_AV_inventories_WP1(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_AV_inventories_WP1(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_AV_inventories_WP1(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			/*DB.find({
		       	selector: {Individu_ID: Individu_ID}
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		row++;
		    			...
		    			put_with_id_AV_inventories_WP1(id, i);
		    			...
		    			search_N_identification_Recursif_AV_inventories_WP1(i-1);
					...
					put_new_id_AV_inventories_WP1(new_doc/*, i*/);
			...
			search_N_identification_Recursif_AV_inventories_WP1(/*localDB, */i-1);
			
		};
	} else {
		
		synchronizeAV_inventories_WP1();
		
	};
};

function put_with_id_AV_inventories_WP1(id, i) {
...
function put_new_id_AV_inventories_WP1(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeAV_inventories_WP1() {
