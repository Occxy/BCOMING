...
var valid_field = 
	['Date', 'Time', 'Individu_ID', 'Common_Name', 'Scientif_Name', 'Sex', 'Age_Adult_Juvenile', 'Weight_g', 'Size_mm', 'Site', 'Degradation_Y_N', 'Nombre_de_filets', 'Prelevement_Sanguin_B', 'Ecouvillon_Oral_OS', 'Ecouvillon_Cloacal_CS'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_AV_Samples(line);
				};
			/*} else {
...
function fill_tab_datas_AV_Samples(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_AV_Samples(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_AV_Samples(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_AV_Samples(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_AV_Samples(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_AV_Samples(/*localDB, */i-1);
...
function put_with_id_AV_Samples(id, i) {
...
function put_new_id_AV_Samples(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeAV_Samples() {
