...
DB.allDocs({  		
	...
	DB2.allDocs({  		
		...
		var rescremer_rongeurs_captures_cameroon = alasql('SELECT latitude, lat_ns, longitude, long_ew, location_code, COUNT(*) AS cremer_rongeurs_captures_cameroon FROM ? GROUP BY latitude, lat_ns, longitude, long_ew, location_code', [tab] );
		
		console.log(rescremer_rongeurs_captures_cameroon)
		for (var j=0;j<rescremer_rongeurs_captures_cameroon.length;j++) {
			tabOut.push(rescremer_rongeurs_captures_cameroon[j]);
		}
		
		interface_publique = JSON.stringify(rescremer_rongeurs_captures_cameroon);
