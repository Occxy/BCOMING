...
var valid_field = 
	['Numero', 'Date', 'Site', 'Def_Site', 'Biotope', 'Observateur', 'Filet', 'Heure_capture', 'Taxon', 'Nom_sc', 'Sexe', 'Avant_bras', 'Doigt_5', 'Doigt_3', 'Pouce', 'Queue', 'Tibia', 'Pied', 'CM3', 'Poids', 'Taille_testicule', 'Taille_epididymes', 'Tunique_vaginale', 'Mammelle', 'Gestation', 'Glandes_taille', 'Glande_couleur', 'Epiphyses', 'Chin_spot', 'Usure_dent', 'Age_estime', 'Code_Individu'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_CH_Samples_data(line);
				};
			/*} else {
...
function fill_tab_datas_CH_Samples_data(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_CH_Samples_data(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_CH_Samples_data(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			Code_Individu = tab[i][31];
			
			DB.find({
		       	selector: {Code_Individu: Code_Individu}
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_CH_Samples_data(id, i);
		    			...
		    			search_N_identification_Recursif_CH_Samples_data(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_CH_Samples_data(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_CH_Samples_data(/*localDB, */i-1);
...
function put_with_id_CH_Samples_data(id, i) {
...
function put_new_id_CH_Samples_data(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeCH_Samples_data() {
