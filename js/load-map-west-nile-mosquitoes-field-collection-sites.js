...
var resWest_nile_mosquitoes_field_collection_sites;
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_west_nile_mosquitoes_field_collection_sites' + debug);
} else {
	var DB = new PouchDB('bcoming_west_nile_mosquitoes_field_collection_sites' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				...
   				obj.Commune = row.doc.Commune.trim() ;
	...
	resWest_nile_mosquitoes_field_collection_sites = alasql('SELECT Lon, Lat, Location, Site, Commune, COUNT(*) AS _Count FROM ? GROUP BY  Lon, Lat, Location, Site, Commune ORDER BY Lon, Lat, Location, Site, Commune', [tab]);
	console.log(resWest_nile_mosquitoes_field_collection_sites);

	var groupedData = {};

	resWest_nile_mosquitoes_field_collection_sites.forEach(function(entry) {
	    var site = entry.Site;
	    ...
	    var key = latitude + '_' + longitude + '_' + site + '_' + location + '_' + commune;
	...
	for (var key in groupedData) {
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	/*var sites = resWest_nile_mosquitoes_field_collection_sites.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    ...
	    contentString += '<p>Location: ' + location + /*', Test count: ' + groupedData[key]*/ ', Commune : ' + commune + '</p>';
	    contentString += '</div>';
