function chargement_insula_mi_samples() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('InsulaMIsamplesTablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Species');
				addValue('Poids_g');
				addValue('Taille_queue_cm');
				addValue('Taille_corps_cm');
				addValue('Sexe');
