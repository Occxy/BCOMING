...
var valid_field = 
	...
	['Id_site', 'Sites_no', 'Sites_proposes2', 'Etat', 'Lat', 'Long'];
...
function fill_tab_datas_sites(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_sites(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_sites(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_sites(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_sites(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_sites(/*localDB, */i-1);
...
function put_with_id_sites(id, i) {
...
function put_new_id_sites(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
