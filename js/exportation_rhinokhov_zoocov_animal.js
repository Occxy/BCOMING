var fields = 
    			['Project', 'Sampling_Date', 'Animal_ID', 'Genus_Species', 'Province', 'District', 'Site', 
    			 'Specific_location', 'Sex', 'Age-Status', 'Health_Status', 'Condition', 'FEV', 'OSV', 'OST',
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DBrhinokhov_zoocov_animal = new PouchDB(remote_couchdb + 'bcoming' + nom_table + debug);
} else {
	var DBrhinokhov_zoocov_animal = new PouchDB('bcoming' + nom_table + debug);
};

var tab_rhinokhov_zoocov_animal = new Array();
var tab = new Array();


DBrhinokhov_zoocov_animal.allDocs({  		
	include_docs: true,
	attachments: true
		...
		tableData.rows.forEach(function(row){   
   			try {
				...
				obj.Other = row.doc.Other
   				...
   				tab_rhinokhov_zoocov_animal.push(obj);
	...
	DBrhinokhov_zoocov_animal.info().then((infos) => {
...
function addRhinoKhov_ZooCov_animalRecord(row, selected) {
		...
		if (!selected) { 	
			...
			if (count == 0) {
				var blob = new Blob(['\ufeff' + CSV_data], {type: "text/csv;charset=ISO-8859-1"});
	       		saveAs(blob, "rhinokhov_zoocov_animal" + clock.now + ".csv");
			}	
...
window.onload = function() {
	$('#export_all_fields').click(function(){
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addRhinoKhov_ZooCov_animalRecord(row, false);
			}
	...
	$('#export_selected_fields').click(function(){
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addRhinoKhov_ZooCov_animalRecord(row, true);
			}
