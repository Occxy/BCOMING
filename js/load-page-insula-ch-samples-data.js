function chargement_insula_ch_samples_data() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('InsulaCHsamples_dataTablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Code_Individu');
				addValue('Numero');
				addValue('Date');
				addValue('Site');
				addValue('Def_Site');
				addValue('Biotope');
				addValue('Observateur');
				addValue('Filet');
				addValue('Heure_capture');
				addValue('Taxon');
				addValue('Nom_sc'); 
				addValue('Sexe'); 
				addValue('Avant_bras');
				...
				addValue('CM3'); 
				addValue('Poids'); 
				addValue('Taille_testicule'); 
				addValue('Taille_epididymes'); 
				addValue('Tunique_vaginale'); 
				addValue('Mammelle'); 
				addValue('Gestation'); 
				addValue('Glandes_taille'); 
				addValue('Glande_couleur'); 
				addValue('Epiphyses'); 
				addValue('Chin_spot'); 
				addValue('Usure_dent'); 
				addValue('Age_estime');
