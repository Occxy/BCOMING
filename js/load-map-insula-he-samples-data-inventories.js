...
var resInsula_he_samples_data_inventories;
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_insula_he_samples_data_inventories' + debug);
} else {
	var DB = new PouchDB('bcoming_insula_he_samples_data_inventories' + debug);
};
...
DB.allDocs({  		
	...
	resInsula_he_samples_data_inventories = alasql('SELECT Quadrat, Site_nom, Lat, Long, COUNT(*) AS _Count FROM ? GROUP BY Quadrat, Site_nom, Lat, Long ORDER BY Lat, Long, Quadrat', [tab]);
	console.log(resInsula_he_samples_data_inventories);

	var groupedData = {};

	resInsula_he_samples_data_inventories.forEach(function(entry) {
	    var quadrat = entry.Quadrat;
	    var latitude = entry.Lat;
	    var longitude = entry.Long;
	    var count = entry._Count;

	    var key = latitude + '_' + longitude + '_' + quadrat;
	...
	for (var key in groupedData) {
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	var sites = resInsula_he_samples_data_inventories.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    ...
	    contentString += '<p>Quadrat: ' + quadrat + ', Count: ' + groupedData[key] + '</p>';
	    contentString += '</div>';
