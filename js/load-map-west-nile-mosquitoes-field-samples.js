...
var resWest_nile_mosquitoes_field_samples;
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_west_nile_mosquitoes_field_samples' + debug);
} else {
	var DB = new PouchDB('bcoming_west_nile_mosquitoes_field_samples' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				var obj = new Object();
   				obj.Site  = row.doc.Site.trim() ;
   				//obj._Count = parseInt(row.doc.Count);    				
   				obj.Location = row.doc.Location.trim() ;	
   				obj.Lon = row.doc.Lon.trim() ;
   				obj.Lat = row.doc.Lat.trim() ;
   				obj.ID_F = row.doc.ID_F.trim() ;
	...
	resWest_nile_mosquitoes_field_samples = alasql('SELECT Lon, Lat, Location, Site,  COUNT(*) AS _Count FROM ? GROUP BY  Lon, Lat, Location, Site ORDER BY Lon, Lat, Location, Site', [tab]);
	resWest_nile_mosquitoes_field_samples_id_f = alasql('SELECT Lon, Lat, Location, Site, ID_F FROM ? ORDER BY ID_F', [tab]);
	console.log(resWest_nile_mosquitoes_field_samples);

	var groupedData = {};

	resWest_nile_mosquitoes_field_samples.forEach(function(entry) {
	    var site = entry.Site;
	    ...
	    var key = latitude + '_' + longitude + '_' + site + '_' + location;
	...
	for (var key in groupedData) {
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	var ID_Fs = resWest_nile_mosquitoes_field_samples_id_f.filter(ID_F => parseFloat(ID_F.Lat) === parseFloat(lastcoords[0]) && parseFloat(ID_F.Lon) === parseFloat(lastcoords[1]));
	    	var ID_FTitle = '';

	    	if (ID_Fs.length > 0) {
	    	    // Utilisez les ID_Fs trouvés pour créer les bulles
	    	    ID_Fs.forEach((ID_F, index) => {
	    	        ID_FTitle += ID_F.ID_F;
	    	        
	    	        // Vérifie si ce n'est pas le dernier élément pour ajouter la virgule
	    	        if (index !== ID_Fs.length - 1) {
	    	            ID_FTitle += ', ';
	    	        }
	    	    ...
	    	    console.log('Aucun ID_F correspondant trouvé');
	    	}
		    ...
		    var infowindow = new google.maps.InfoWindow({
		    	content: '<div style="background-color: lightgreen; color: dark;"><strong>' + lastLocation + '</strong><br>' + ID_FTitle + '</div>'
		 	});
	    ...
	    contentString += '<p>Site: ' + site + ', Test count: ' + groupedData[key] + '</p>';
	    contentString += '</div>';
