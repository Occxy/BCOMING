...
var resBcoming_guinea_survey_data;

var centerCoords = { lat: 7.956590176, lng: -8.619250298 };
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_merfi_guinea_survey_data' + debug);
} else {
	var DB = new PouchDB('bcoming_merfi_guinea_survey_data' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				var obj = new Object();
   				obj.Result_Id  = row.doc.Result_Id.trim() ;
   				//obj._Count = parseInt(row.doc.Count);    				
   				obj.Surveyed_Date = row.doc.Surveyed_Date.trim() ;	
   				obj.Lon = row.doc.Location_Longitude.trim() ;
   				obj.Lat = row.doc.Location_Latitude.trim() ;
	...
	resBcoming_guinea_survey_data = alasql('SELECT Lon, Lat, Surveyed_Date, Result_Id FROM ? GROUP BY  Lon, Lat, Surveyed_Date, Result_Id ORDER BY Lon, Lat, Surveyed_Date, Result_Id', [tab]);
	console.log(resBcoming_guinea_survey_data);

	var groupedData = {};

	resBcoming_guinea_survey_data.forEach(function(entry) {
		var survey_date = entry.Surveyed_Date;
		var result_id = entry.Result_Id;
	    var latitude = entry.Lat;
	    var longitude = entry.Lon;
	    //var count = entry._Count;

	    var key = latitude + '_' + longitude + '_' + survey_date + '_' + result_id;

	    if (!(key in groupedData)) {
	        groupedData[key] = 0;
	    }

	    groupedData[key] += parseInt(result_id);
	});
	...
	for (var key in groupedData) {
	    ...
	    var survey_date = coords[2];
	    var result_id = coords[3];
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	/*var sites = resBcoming_guinea_survey_data.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    	    // Utilisez les sites trouvés pour créer les bulles
	    	    sites.forEach(site => {
	    	        siteTitle = site.Site_result_id + '/' + site.Lat + '/' + site.Long;
		    ...
		    var infowindow = new google.maps.InfoWindow({
		    	content: '<div style="background-color: lightgreen; color: dark;"><strong>' + lastResult_Id + '</strong><br>' + contentString + '</div>'
		 	});
	    ...
	    contentString += '<p>Survey_Date: ' + survey_date + '</p>';
	    contentString += '</div>';
