...
DB.allDocs({  		
	...
	DB2.allDocs({  		
		...
		var resInsula_sites = alasql('SELECT Lat, Long, Id_site, COUNT(*) AS Insula_sites FROM ? GROUP BY Lat, Long, Id_site', [tab] );
		
		console.log(resInsula_sites)
		for (var j=0;j<resInsula_sites.length;j++) {
			tabOut.push(resInsula_sites[j]);
		}
		
		interface_publique = JSON.stringify(resInsula_sites);
