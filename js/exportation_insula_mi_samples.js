var fields = 
    			...
    			['Sample', 'Date', 'Observateurs', 'Site', 'Def_site', 'Biotope', 'Taxa', 'Famille', 'Genre', 'Species', 'Poids_g', 
    			 'Taille_queue_cm', 'Taille_corps_cm', 'Sexe', 'Code', 'Username'] 
    			 
    		


var fields_CSV_head =   'Sample;Date;Observateurs;Site;Def_site;Biotope;Taxa;Famille;Genre;Species;Poids_g;' +
						'Taille_queue_cm;Taille_corps_cm;Sexe;Code;Username;\r\n';
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DBinsula_mi_samples = new PouchDB(remote_couchdb + 'bcoming' + nom_table + debug);
} else {
	var nom_table_mi = '_insula_mi_samples' + debug;
	var DBinsula_mi_samples = new PouchDB('bcoming' + nom_table_mi + debug);
};

var tab_insula_mi_samples = new Array();
var tab = new Array();


DBinsula_mi_samples.allDocs({  		
	include_docs: true,
	attachments: true
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				...
   				obj.Sample = row.doc.Sample 
   				obj.Date = row.doc.Date 
   				obj.Observateurs = row.doc.Observateurs
   				...
   				obj.Taille_queue_cm = row.doc.Taille_queue_cm 
   				obj.Taille_corps_cm = row.doc.Taille_corps_cm 
   				obj.Sexe = row.doc.Sexe 
   				obj.Code = row.doc.Code
   				obj.Username = row.doc.Username

   				tab_insula_mi_samples.push(obj);
	...
	DBinsula_mi_samples.info().then((infos) => {
...
function addInsula_mi_samplesRecord(row, selected) {
		...
		if (!selected) { 	
			...
			if (count == 0) {
				var blob = new Blob(['\ufeff' + CSV_data], {type: "text/csv;charset=ISO-8859-1"});
	       		saveAs(blob, "insula_mi_samples" + clock.now + ".csv");
	       		document.location.href="import_export.html?table=_insula_mi&type=insula_mi"; 
			}	
...
window.onload = function() {
	...
	$('#export_all_fields').click(function(){
		...
		var res = alasql("SELECT Sample, Date, Observateurs, Site, Def_site, Biotope, Taxa, Famille, Genre, Species, Poids_g, Taille_queue_cm, Taille_corps_cm, Sexe, Code, Username FROM ? ORDER BY Date", [tab_insula_mi_samples] );
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addInsula_mi_samplesRecord(row, false);
			}
	...
	$('#export_selected_fields').click(function(){
	    ...
	    res.forEach(function(row){
			...
			//alert(count)
			if (count == i) {
				addInsula_mi_samplesRecord(row, true);
			}
