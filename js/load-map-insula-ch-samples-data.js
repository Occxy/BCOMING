...
DB.allDocs({  		
	...
	DB2.allDocs({  		
		...
		var resInsula_ch_samples_data = alasql('SELECT Site, Def_Site, COUNT(*) AS count_ch_samples_data FROM ? GROUP BY Site, Def_Site', [tab2] );
		console.log(resInsula_ch_samples_data)
		var query = `
		    SELECT 
		        resInsula_sites.Lat, 
		        ...
		        resInsula_ch_samples_data.Site, 
		        resInsula_ch_samples_data.Def_Site, 
		        resInsula_ch_samples_data.count_ch_samples_data
		    FROM ? AS resInsula_sites
		    INNER JOIN ? AS resInsula_ch_samples_data ON 
				UPPER(
				    REPLACE(
				        REPLACE(
				            REPLACE(
				    ...
				    REPLACE(
				        REPLACE(
				            REPLACE(
				                REPLACE(resInsula_ch_samples_data.Site, 'à', 'a'), 'é', 'e'), 'ï', 'i'), 'ô', 'o')
				) AND 
				UPPER(
				    REPLACE(
				        REPLACE(
				    ...
				    REPLACE(
				        REPLACE(
				            REPLACE(
				                REPLACE(resInsula_ch_samples_data.Def_Site, 'à', 'a'), 'é', 'e'), 'ï', 'i'), 'ô', 'o')
				);
		`;
		
		var joinedResult = alasql(query, [resInsula_sites, resInsula_ch_samples_data]);
	   		...
	   		try {	
	   			...
	   			if (index === (obj_interface_publique.length -1)) {
					...
					lastLong = row.Long;
   					...
   					lastNumberOfIndividuals = row.count_ch_samples_data;
	   				/*lastDate = row.Date;
	   				...
	   				if ((isFloat(_lat)) && (isFloat(_lng))) {
						marker_list.push(new ol.Feature({
		  	           		...
		  	           		datas: lastCadre,
		  	           	    Id_site: lastId_site,
		  	           	    NumberOfIndividuals: lastNumberOfIndividuals,
		...
		map.on('singleclick', function(evt) {
			var name = map.forEachFeatureAtPixel(evt.pixel, function(feature) {
				...
				//return feature.get('Id_site') + '\r\n Number of individuals : ' + feature.get('NumberOfIndividuals') + '\r\n' + feature.get('lat') + '\r' + feature.get('lng') /*+ '\r' +*/
					   //feature.get('espece');
				//return feature.get('datas')
				
				var numberOfIndividuals = feature.get('NumberOfIndividuals');
		        var contentHTML = '<div>';
		        contentHTML += feature.get('Id_site') + '<br>';
		        contentHTML += '<span style="color: red;">Number of individuals: ' + numberOfIndividuals + '</span><br>';
		        contentHTML += feature.get('lat') + '<br>';
		        contentHTML += feature.get('lng') + '<br>';
		        contentHTML += '</div>';
