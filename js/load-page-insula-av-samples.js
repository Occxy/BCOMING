function chargement_insula_av_samples() {
	showConnexionStatus();
...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('InsulaAVsamplesTablesData'));
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				addValue('Time');
				addValue('Individu_ID');
				addValue('Common_Name');
				addValue('Scientif_Name');
				addValue('Sex');
				addValue('Age_Adult_Juvenile');
				addValue('Weight_g');
				addValue('Size_mm');
				addValue('Site');
				addValue('Degradation_Y_N');
				addValue('Nombre_de_filets');
				addValue('Prelevement_Sanguin_B');
				addValue('Ecouvillon_Oral_OS');
				addValue('Ecouvillon_Cloacal_CS');
