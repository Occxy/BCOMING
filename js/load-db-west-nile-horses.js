var west_nile_horses_table = localStorage.getItem('west_nile_horses_table');
...
$(document).ready(function() {			
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + west_nile_horses_table + debug);
	} else {
		
		var DB = new PouchDB(west_nile_horses_table + debug);
	};	
  	DB.allDocs({
		include_docs: true,
		attachments: true
		...
		if (typeof(JSON.stringify(result)) != "undefined"){  
    		...
    		dataTablesData.rows.forEach(function(row){   
    			table.row.add(new WestNileHorses(row.doc.Date_prelevement, row.doc.Nom, row.doc.Club, row.doc.N_de_suivi, row.doc.N_de_res,  
...
function WestNileHorses(Date_prelevement, Nom, Club, N_de_suivi, N_de_res, Action) {
