...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_west_nile_chickens' + debug);
} else {
	var DB = new PouchDB('bcoming_west_nile_chickens' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
 			try {
 				var obj = new Object();
 				obj.Description  = row.doc.Description.trim() ;
 				//obj._Count = parseInt(row.doc.Count);    				
 				obj.Commune = row.doc.Commune.trim() ;	
 				/*obj.Lon = parseFloat(row.doc.Lon.trim()).toFixed(3) ;
 				obj.Lat = parseFloat(row.doc.Lat.trim()).toFixed(3) ;*/
 				obj.Lon = parseFloat(row.doc.Lon.trim()) ;
 				obj.Lat = parseFloat(row.doc.Lat.trim()) ;
	...
	resWest_nile_chickens = alasql('SELECT Lon, Lat, Commune, Description,  COUNT(*) AS _Count FROM ? GROUP BY  Lon, Lat, Commune, Description ORDER BY Lon, Lat, Commune, Description', [tab]);
	console.log(resWest_nile_chickens);

	var groupedData = {};

	resWest_nile_chickens.forEach(function(entry) {
	    ...
	    var key = latitude + '_' + longitude + '_' + description + '_' + commune;
	...
	for (var key in groupedData) {
	    ...
	    var description = coords[2];
	    var commune = coords[3];
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	/*var sites = resWest_nile_chickens.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    	    // Utilisez les sites trouvés pour créer les bulles
	    	    sites.forEach(site => {
	    	        siteTitle = site.Site_description + '/' + site.Lat + '/' + site.Long;
		    ...
		    var infowindow = new google.maps.InfoWindow({
		    	content: '<div style="background-color: lightgreen; color: dark;"><strong>' + lastCommune + '</strong><br>' + contentString + '</div>'
		 	});
	    ...
	    contentString += '<p>Description: ' + description + ', Number of samples: ' + groupedData[key] + '</p>';
	    contentString += '</div>';
