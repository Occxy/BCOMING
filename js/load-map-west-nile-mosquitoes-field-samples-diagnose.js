...
var resWest_nile_mosquitoes_field_samples_diagnose;
...
if (localStorage.getItem('web') === 'oui') {
	...
	var DB = new PouchDB(remote_couchdb + 'bcoming_west_nile_mosquitoes_field_samples_diagnose' + debug);
} else {
	var DB = new PouchDB('bcoming_west_nile_mosquitoes_field_samples_diagnose' + debug);
};
...
DB.allDocs({  		
	...
	if (typeof(JSON.stringify(result)) != "undefined"){  
		...
		tableData.rows.forEach(function(row){   
   			try {
   				...
   				obj.Individuals = parseInt(row.doc.Individuals);    				
	...
	resWest_nile_mosquitoes_field_samples_diagnose = alasql('SELECT Lon, Lat, Location, Site, SUM(Individuals) AS Sum_Individuals FROM ? GROUP BY  Lon, Lat, Location, Site ORDER BY Lon, Lat, Location, Site', [tab]);
	console.log(resWest_nile_mosquitoes_field_samples_diagnose);

	var groupedData = {};

	resWest_nile_mosquitoes_field_samples_diagnose.forEach(function(entry) {
	    var site = entry.Site;
	    var location = entry.Location;
	    var latitude = entry.Lat;
	    var longitude = entry.Lon;
	    var sum_individuals = entry.Sum_Individuals;
	    ...
	    groupedData[key] += parseInt(sum_individuals);
	});
	...
	for (var key in groupedData) {
	    ...
	    if ((contentString  != '') && (lat_lng != coords[0]+'_'+coords[1])) {
	    	...
	    	/*var sites = resWest_nile_mosquitoes_field_samples_diagnose.filter(site => parseFloat(site.Lat) === parseFloat(lastcoords[0]) && parseFloat(site.Long) === parseFloat(lastcoords[1]));
	    	if (sites.length > 0) {
	    	    // Utilisez les sites trouvés pour créer les bulles
	    	    sites.forEach(site => {
	    	        siteTitle = site.Site_site + '/' + site.Lat + '/' + site.Long;
		    ...
		    var infowindow = new google.maps.InfoWindow({
		    	content: '<div style="background-color: lightgreen; color: dark;"><strong>' + lastSite + '</strong><br>' + contentString + '</div>'
		 	});
	    ...
	    if (groupedData[key] == 0) {
	    	contentString += '<p>Location: ' + location + ', Individuals: undefined</p>';
	    } else {
	    	contentString += '<p>Location: ' + location + ', Individuals: ' + groupedData[key] + '</p>';
	    }
	    contentString += '</div>';
