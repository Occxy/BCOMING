...
var valid_field = 
	['Individus', 'Salive', 'Excrements', 'Sang', 'Biopsie', 'Espece', 'Site', 'Date', 'Note'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_CH_Samples(line);
				};
			/*} else {
...
function fill_tab_datas_CH_Samples(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_CH_Samples(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_CH_Samples(/*localDB,*/ i, j, k, N_site, row) {
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_CH_Samples(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_CH_Samples(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_CH_Samples(/*localDB, */i-1);
...
function put_with_id_CH_Samples(id, i) {
...
function put_new_id_CH_Samples(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeCH_Samples() {
