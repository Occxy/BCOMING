...
var valid_field = 
	...
	['Quadrat', 'Note', 'Genre', 'Sp', 'Recouv', 'Site_ID', 'Site_nom', 'Code', 'Lat', 'Long', 'Date'];
		...
		if (fields_is_valid()) {
			...
			//if (is_only_one_country()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_HE_Samples_data_inventories(line);
				};
			/*} else {
...
function fill_tab_datas_HE_Samples_data_inventories(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_HE_Samples_data_inventories(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_HE_Samples_data_inventories(/*localDB,*/ i, j, k, N_site, row) {
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			DB.find({
		       	selector: {Code: /*ne pas tester le filtre*/ 'Code'}
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			put_with_id_HE_Samples_data_inventories(id, i);
		    			...
		    			search_N_identification_Recursif_HE_Samples_data_inventories(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
					...
					put_new_id_HE_Samples_data_inventories(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_HE_Samples_data_inventories(/*localDB, */i-1);
		...
		synchronizeHE_Samples_data_inventories();
		/*if (localStorage.getItem('web') !== 'oui') {
...
function put_with_id_HE_Samples_data_inventories(id, i) {
...
function put_new_id_HE_Samples_data_inventories(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'bcoming' + import_table + debug);
	} else {
...
function synchronizeHE_Samples_data_inventories() {
